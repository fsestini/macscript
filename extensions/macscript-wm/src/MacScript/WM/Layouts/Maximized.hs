module MacScript.WM.Layouts.Maximized where
...
handleStdMsgs :: StdMessage -> [TiledWindow] -> WM (Maybe [TiledWindow])
handleStdMsgs (InsertNew tw) tws = pure (Just (tw : tws))
handleStdMsgs (FocusTowards w dir) tws =
  (>> pure Nothing) . maybe (pure ()) (justLog "focusing window" . focusWindow) $ case dir of
